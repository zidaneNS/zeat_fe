    ...
    return (
        <form action={action} className="flex flex-col gap-y-4 bg-orange-300 rounded-md shadow-xl py-4 px-6 w-1/2 text-orange-800">
            ...
            <FormInputField
                ...
                title="Email"
            />
            {state?.errors?.email && <ErrorMsg errMsg={state.errors?.email} />}
            <FormInputField
                type="password"
                name="password"
            ...
            {pending ? (
                ...
                <button className="w-full bg-indigo-700 rounded-md shadow-xl text-white cursor-pointer hover:bg-indigo-500 duration-300 text-center py-2">Sign In</button>
            )}
